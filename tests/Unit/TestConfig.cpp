...
using namespace LibIndexer::Config;

namespace {
Inpx::BookRecord MakeRec(const std::string& lang="ru",
...
{
    REQUIRE_THROWS_AS(CAppConfig::Load("/no/such.json"), CConfigError);
}
...
{
    REQUIRE(CBookFilter(FiltersConfig{}).ShouldInclude(MakeRec()));
}
TEST_CASE("CBookFilter excludeLanguages", "[config]")
{
    FiltersConfig f; f.excludeLanguages = {"en"};
    CBookFilter bf(f);
...
{
    FiltersConfig f; f.includeLanguages = {"ru"};
    CBookFilter bf(f);
    REQUIRE(bf.ShouldInclude(MakeRec("ru")));
    REQUIRE_FALSE(bf.ShouldInclude(MakeRec("en")));
}
...
{
    FiltersConfig f; f.minFileSize = 50000;
    CBookFilter bf(f);
