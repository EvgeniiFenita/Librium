...
TEST_CASE("Config::Utf8ToPath", "[config]")
{
    ...
    {
        auto p = Utils::CStringUtils::Utf8ToPath("hello/world.txt");
    ...
    {
        auto p = Utils::CStringUtils::Utf8ToPath("");
        REQUIRE(p.empty());
    }
    ...
    SECTION("Cyrillic UTF-8 path is preserved")
    {
        ...
        auto p = Utils::CStringUtils::Utf8ToPath(utf8);
    ...
    {
        auto p = Utils::CStringUtils::Utf8ToPath("my books/war and peace.fb2");
        REQUIRE(p.filename() == "war and peace.fb2");
    }

    SECTION("Dot and dotdot components are preserved")
    {
        auto p = Utils::CStringUtils::Utf8ToPath("./relative/path.db");
        REQUIRE_FALSE(p.empty());
    }
}
...
TEST_CASE("Config::GetBookMetaDir", "[config]")
{
    SECTION("Builds path: <dbDir>/meta/<id>")
    {
        ...
        auto metaDir = Config::CAppPaths::GetBookMetaDir(dbPath, 42);
    ...
    SECTION("Id is used verbatim as directory name")
    {
        ...
        REQUIRE(Config::CAppPaths::GetBookMetaDir(dbPath, 1).filename()   == "1");
        REQUIRE(Config::CAppPaths::GetBookMetaDir(dbPath, 999).filename() == "999");
    }

    SECTION("Different ids produce different paths")
    {
        std::filesystem::path dbPath = "lib.db";
        auto dir1 = Config::CAppPaths::GetBookMetaDir(dbPath, 1);
        auto dir2 = Config::CAppPaths::GetBookMetaDir(dbPath, 2);
        REQUIRE(dir1 != dir2);
    }
    ...
    SECTION("Parent is always the 'meta' directory next to db")
    {
        ...
        auto metaDir = Config::CAppPaths::GetBookMetaDir(dbPath, 7);
    ...
    SECTION("Works with absolute path to database")
    {
        ...
        auto metaDir = Config::CAppPaths::GetBookMetaDir(dbPath, 100);
