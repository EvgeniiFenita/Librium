...
- **Stages**: 
  1. **Unit**: Fast C++ tests via Catch2. Self-contained, no external scripts. Covers filters, encoding, Base64, concurrency, transactions, INPX (streaming & edge cases), ZIP (RAII & edge cases), FB2 (cover extraction & encoding edge cases), logger, search query parser, database `get-book` fields, database query logic, SQLite custom functions, indexer pipeline, string sanitization, config (utilities & edge cases), version parsing.
