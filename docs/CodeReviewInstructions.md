...
- **Inline SQL Strings**: All SQL — including queries (`SELECT`, `INSERT`, `UPDATE`, `DELETE`, `CREATE TABLE`), transaction control (`BEGIN TRANSACTION`, `COMMIT`, `ROLLBACK`), and SQLite PRAGMAs (`cache_size`, `mmap_size`, `temp_store`, `busy_timeout`, `journal_mode`, `synchronous`) — must live in `SqlQueries.hpp` as `constexpr std::string_view` constants. **Search strategy**: grep all files outside `SqlQueries.hpp` for string literals containing `SELECT`, `INSERT`, `UPDATE`, `DELETE`, `CREATE TABLE`, `PRAGMA`, `BEGIN`, `COMMIT`, or `ROLLBACK` — any match is a violation.
