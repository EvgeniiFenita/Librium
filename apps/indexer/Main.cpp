...
static void SetupLogging(const Config::LoggingConfig& cfg,
...
int main(int argc, char* argv[])
{
    ...
    CLI::App app{"libindexer - INPX library indexer"};
    app.set_version_flag("--version,-v",
        std::string("libindexer v") + Apps::Indexer::VersionString);
    ...
    importCmd->add_option("--config", importConfig, "Config JSON path")->required();
    ...
    const std::string& configPath = isUpgrade ? upgradeConfig : importConfig;
    ...
    Log::CLogger::Instance().Info(
        "libindexer v{} — mode: {}", Apps::Indexer::VersionString, cfg.import.mode);
